"""
Link5: RCM 관리 상세 E2E 테스트

[체크리스트 파일] test/e2e_checklist_link5.md
...
실행 방법:
    python test/test_e2e_link5.py --url http://localhost:5001
"""
...
from test.playwright_base import PlaywrightTestBase, E2ETestResult, TestStatus
...
class Link5RCMTestSuite(PlaywrightTestBase):
    ...
    def __init__(self, base_url="http://localhost:5001", headless=False):
        ...
        self.checklist_source = project_root / "test" / "e2e_checklist_link5.md"
        self.checklist_result = project_root / "test" / "e2e_checklist_link5_result.md"
        self.test_rcm_name = f"E2E_Test_RCM_{datetime.now().strftime('%H%M%S')}"
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            self.run_category("2. 목록 및 권한", [
                ...
                self.test_rcm_data_isolation
            ])

            # 카테고리 3: 상세 및 수정
            self.run_category("3. 상세 및 수정", [
                self.test_rcm_detail_mapping,
                self.test_rcm_attribute_setting
            ])
    ...
    def test_rcm_upload_success(self, result: E2ETestResult):
        """정상 엑셀 파일 업로드 성공"""
        # Step 1: 관리자 로그인
    ...
    def test_rcm_upload_invalid_ext(self, result: E2ETestResult):
        """잘못된 파일 형식 거부"""
    ...
    def test_rcm_upload_missing_required(self, result: E2ETestResult):
        """필수 데이터 누락 처리"""
        self._do_admin_login()
        self._navigate_to_rcm_list()
        self._click_rcm_upload_button()
    ...
    def test_rcm_list_metadata(self, result: E2ETestResult):
        """업로드된 RCM 메타데이터 표시"""
        self._do_admin_login()
        self._navigate_to_rcm_list()
    ...
    def test_rcm_data_isolation(self, result: E2ETestResult):
        """타 사용자 데이터 격리 (현재 관리자 계정에서는 모든 데이터 보임)"""
        # 참고: 관리자 로그인 시 모든 RCM이 보이므로 이 테스트는 제한적
        result.skip_test("관리자 로그인은 모든 데이터 접근 가능 (다중 사용자 테스트 필요)")

    # =========================================================================
    # 3. 상세 및 수정 테스트
    # =========================================================================

    def test_rcm_detail_mapping(self, result: E2ETestResult):
        """상세 데이터 매핑 정확성"""
        self._do_admin_login()
        self._navigate_to_rcm_list()
    ...
    def test_rcm_attribute_setting(self, result: E2ETestResult):
        """Attribute(모집단 필드 정의) 설정 및 저장 확인"""
        try:
            setup_btn = self.page.locator("button:has-text('설정'), button:has-text('Attribute')").first

            if setup_btn.count() == 0:
                result.skip_test("Attribute 설정 버튼을 찾을 수 없음")
                return

            setup_btn.click()

            try:
                self.page.wait_for_selector("#attributeModal", state="visible", timeout=3000)
            except:
                result.skip_test("Attribute 설정 모달이 열리지 않음")
                return

            attr_input_0 = self.page.locator("#attr-input-0")
            if attr_input_0.count() > 0:
                attr_input_0.fill("전표번호")

            attr_input_1 = self.page.locator("#attr-input-1")
            if attr_input_1.count() > 0:
                attr_input_1.fill("일자")

            pop_count = self.page.locator("#population-attr-count")
            if pop_count.count() > 0:
                pop_count.fill("2")

            save_btn = self.page.locator("#attributeModal button:has-text('저장')")
            if save_btn.count() > 0:
                save_btn.click()
                self.page.wait_for_timeout(1000)

                if self.page.locator("text=저장, text=2개 설정됨").count() > 0:
                    result.pass_test("Attribute 설정 저장 확인")
                else:
                    result.warn_test("저장은 된 것 같으나 UI 확인 불가")
            else:
                result.skip_test("Attribute 저장 버튼을 찾을 수 없음")

        except Exception as e:
            result.fail_test(f"Attribute 설정 테스트 실패: {e}")

    # =========================================================================
    # 4. 삭제 테스트
    # =========================================================================

    def test_rcm_delete(self, result: E2ETestResult):
        """RCM 삭제 수행"""
        self._do_admin_login()
        self._navigate_to_rcm_list()
...
def main():
    parser = argparse.ArgumentParser(description='Link5 RCM Detailed E2E Test')
