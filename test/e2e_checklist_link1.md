...
- [ ] **test_link1_activity_log**: 페이지 접근 시 `PAGE_ACCESS` 로그가 활동 기록에 정상적으로 남는지 확인
...
- [ ] **test_rcm_mail_attachment**: 실제 수신된 메일에 생성된 파일명(`[시스템명]_ITGC_RCM_[날짜].xlsx`)이 규칙에 맞게 첨부되어 있는지 확인
- [ ] **test_rcm_mail_failure_handling**: 메일 서버 오류 등 예외 발생 시 `error.jsp`로 이동하며 적절한 에러 메시지를 보여주는지 확인

## 5. 활동 로그 및 데이터 확인
- [ ] **test_rcm_generation_log**: 메일 발송 성공 후 `user_activity`에 관련 기록(RCM 생성 요청 등)이 남는지 확인
