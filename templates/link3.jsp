        ...
        function enableStepByStep(type) {
            // 각 항목별 step 데이터 정의
                ...
                APD02: [
                    ...
                    {img: "/static/img/step3.jpg", title: "Step 3: 권한 회수 여부 점검", desc: "1. 부서이동일 이후 기존 권한이 모두 회수되었는지 실제로 회수 내역을 점검합니다.<br>2. 모든 사용자가 보유하고 있는 기본권한 등은 회수회지 않아도 무방합니다.<br>3. 회수되지 않은 권한이 있다면 즉시 조치합니다."}
                ],
                APD03: [
                    {img: "/static/img/step1.jpg", title: "Step 1: 모집단 확인", desc: "1. 해당 통제의 모집단(예: 퇴사자 명단)을 시스템 또는 인사자료에서 추출합니다.<br>2. 추출 시점의 데이터 건수와 캡쳐 화면을 확보하여 완전성을 확인합니다.<br>3. 전체 데이터와 캡쳐된 화면을 운영평가 조서에 작성합니다."},
                    {img: "/static/img/step2.jpg", title: "Step 2: 샘플 선정", desc: "1. 모집단 수에 따라 샘플 수를 결정합니다. (예: 모집단이 10개인 경우 Quarterly와 Monthly 사이이므로 2개, 13개인 경우 Monthly와 Weekly 사이이므로 5개 선정 등)<br>2. 샘플 선정은 무작위 표본추출(Simple Random Sampling) 방식으로 해야 하며, 임의의 데이터를 선택하면 안됩니다.<br>3. 선정된 샘플만 운영평가 조서에 별도로 작성합니다."},
                    {img: "/static/img/step3.jpg", title: "Step 3: 권한 회수 여부 점검", desc: "1. 퇴사일 이후 접근 권한이 모두 회수되었는지 실제로 회수 내역을 점검합니다.<br>2. 회수되지 않은 권한이 있다면 즉시 조치합니다."}
                ],
                APD04: [
                ...
                APD06: [
                    {img: "/static/img/step1.jpg", title: "Step 1: 모집단 확인", desc: "1. 해당 통제의 모집단(예: 데이터 변경 이력)을 시스템에서 추출합니다.<br>2. 추출 시점의 데이터 건수와 캡쳐 화면을 확보하여 완전성을 확인합니다.<br>3. DB에 접속하여 쿼리를 통해 데이터를 변경한 내역(Insert, Update, Delete)을 대상으로 하며 시스템이 생성한 쿼리는 제외합니다."},
                    {img: "/static/img/step2.jpg", title: "Step 2: 샘플 선정", desc: "1. 모집단 수에 따라 샘플 수를 결정합니다. (예: 모집단이 10개인 경우 Quarterly와 Monthly 사이이므로 2개, 13개인 경우 Monthly와 Weekly 사이이므로 5개 선정 등)<br>2. 샘플 선정은 무작위 표본추출 방식(Simple Random Sampling)으로 해야 하며, 임의의 데이터를 선택하면 안됩니다.<br>3. 선정된 샘플만 운영평가 조서에 별도로 작성합니다."},
                ...
                APD07: [
                    {img: "/static/img/step1.jpg", title: "Step 1: 모집단 확인", desc: "1. 해당 통제의 모집단(예: DB 접근권한 부여 이력)을 시스템에서 추출합니다.<br>2. 추출 시점의 데이터 건수와 캡쳐 화면을 확보하여 완전성을 확인합니다."},
                    {img: "/static/img/step2.jpg", title: "Step 2: 샘플 선정", desc: "1. 모집단 수에 따라 샘플 수를 결정합니다. (예: 모집단이 10개인 경우 Quarterly와 Monthly 사이이므로 2개, 13개인 경우 Monthly와 Weekly 사이이므로 5개 선정 등)<br>2. 샘플 선정은 무작위 표본 추출(Simple Random Sampling) 방식으로 해야 하며, 임의의 데이터를 선택하면 안됩니다.<br>3. 선정된 샘플만 운영평가 조서에 별도로 작성합니다."},
                ...
                APD10: [
                    {img: "/static/img/step1.jpg", title: "Step 1: 모집단 확인", desc: "1. 해당 통제의 모집단(예: OS 접근권한 부여 이력)을 시스템에서 추출합니다.<br>2. 추출 시점의 데이터 건수와 캡쳐 화면을 확보하여 완전성을 확인합니다."},
                    {img: "/static/img/step2.jpg", title: "Step 2: 샘플 선정", desc: "1. 모집단 수에 따라 샘플 수를 결정합니다. (예: 모집단이 10개인 경우 Quarterly와 Monthly 사이이므로 2개, 13개인 경우 Monthly와 Weekly 사이이므로 5개 선정 등)<br>2. 샘플 선정은 무작위 표본 추출(Simple Random Sampling) 방식으로 해야 하며, 임의의 데이터를 선택하면 안됩니다.<br>3. 선정된 샘플만 운영평가 조서에 별도로 작성합니다."},
                    {img: "/static/img/step3.jpg", title: "Step 3: 권한 부여 내역 기록 점검", desc: "1. 선정된 샘플에 대한 증빙(승인 이력)을 확인합니다.<br>2. 권한 부여 일자와 승인 일자를 대사하여 사전 승인 여부를 확인합니다."}
                ],
                APD11: [
                ...
                PC01: [
                    {img: "/static/img/step1.jpg", title: "Step 1: 모집단 확인", desc: "1. 해당 통제의 모집단(예: 프로그램 이관 이력)을 시스템에서 추출합니다.<br>2. 추출 시점의 데이터 건수와 캡쳐 화면을 확보하여 완전성을 확인합니다."},
                    {img: "/static/img/step2.jpg", title: "Step 2: 샘플 선정", desc: "1. 모집단 수에 따라 샘플 수를 결정합니다. (예: 모집단이 10개인 경우 Quarterly와 Monthly 사이이므로 2개, 13개인 경우 Monthly와 Weekly 사이이므로 5개 선정 등)<br>2. 샘플 선정은 무작위 표본추출(Simple Random Sampling) 방식으로 해야 하며, 임의의 데이터를 선택하면 안됩니다.<br>3. 선정된 샘플만 운영평가 조서에 별도로 작성합니다."},
                ...
                PC05: [
                    {img: "/static/img/step1.jpg", title: "Step 1: 개발/운영 환경 분리 현황 확인", desc: "1. 시스템이 운영환경과 별도의 개발환경을 보유하고 있는지 확인합니다."},
                    {img: "/static/img/step2.jpg", title: "Step 2: 증빙 자료 확보", desc: "1. 서버 구성도, IP 목록 등으로 개발환경과 운영환경이 분리되어 있는지 증빙 자료를 확보합니다."},
                    {img: "/static/img/step3.jpg", title: "Step 3: 환경 분리 적정성 검토", desc: "1. 확보한 증빙 자료를 바탕으로 개발/운영 환경 분리가 적정하게 이루어졌는지 검토합니다."}
                ],
                PC06: [
                    {img: "/static/img/step1.jpg", title: "Step 1: 모집단 확인", desc: "1. 당기 DB 패치 이력을 모집단으로 선정합니다.<br>2. 패치 이력 전체를 시스템에서 추출하고, 추출 시점의 데이터 건수와 캡쳐 화면을 확보하여 완전성을 확인합니다."},
                ...
                CO02: [
                    {img: "/static/img/step1.jpg", title: "Step 1: 배치잡 등록 권한 사용자 추출", desc: "1. 배치잡 등록 권한을 보유한 사용자를 시스템에서 추출합니다.<br>2. 추출 시점의 사용자 목록과 캡쳐 화면을 확보하여 완전성을 확인합니다."},
                    {img: "/static/img/step2.jpg", title: "Step 2: 부서/직무 등 검토", desc: "1. 추출된 사용자의 부서, 직무, 담당 업무 등을 검토하여 배치잡 등록 권한 보유의 적정성을 확인합니다."},
                    {img: "/static/img/step3.jpg", title: "Step 3: 적정성 판단 및 조치", desc: "1. 불필요하거나 부적정한 배치잡 등록 권한이 확인될 경우 즉시 권한을 제한하고, 그 내역을 기록합니다."}
                ],
                CO03: [
