        ...
        function loadAvailableRcmCount() {
            fetch('/api/user/rcm-list')
                ...
                .then(data => {
                    if (data.success) {
                        // TLC RCM만 필터링하여 카운트
                        const tlcRcms = data.rcms.filter(rcm => rcm.control_category === 'TLC');
                        const tlcCount = tlcRcms.length;

                        document.getElementById('availableRcmCount').textContent = tlcCount;
                        ...
                        if (tlcCount === 0) {
                            ...
                            startButton.innerHTML = '<i class="fas fa-exclamation-triangle me-1"></i>평가 가능한 TLC RCM 없음';
            ...
            fetch('/api/user/rcm-list')
                ...
                .then(data => {
                    if (data.success && data.rcms.length > 0) {
                        ...
                        const tlcRcms = data.rcms.filter(rcm => rcm.control_category === 'TLC');
                        if (tlcRcms.length > 0) {
                            showRcmSelectionModal(tlcRcms);
                        } else {
                            alert('평가할 수 있는 TLC RCM이 없습니다.');
                        }
                    } else {
            ...
            fetch('/api/user/rcm-list')
                ...
                .then(data => {
                    if (data.success && data.rcms.length > 0) {
                        ...
                        const tlcRcms = data.rcms.filter(rcm => rcm.control_category === 'TLC');
                        if (tlcRcms.length > 0) {
                            showRcmSelectionModal(tlcRcms, false);
                        } else {
                            alert('평가할 수 있는 TLC RCM이 없습니다.');
                        }
                    } else {
