# snowball_link2_1page.py
# 섹션별(4페이지) ITGC 인터뷰 시스템
# 기존 snowball_link2.py의 단계별(52페이지) 방식 대신
...
def reset_1p_session():
    """1page 인터뷰 세션 완전 초기화"""
    for key in ['link2_1p_answers', 'link2_1p_textarea', 'link2_1p_section']:
...
def section_view(section_name):
    ...
    if request.method == 'POST':
        ...
        for idx in range(sec_info['q_start'], sec_info['q_end'] + 1):
            # 폼 필드명: q{idx} (메인), q{idx}_text (textarea)
